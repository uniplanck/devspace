...
const SHARED_PULL_FAILURE_RETRY_MS = 5_000;
...
export type OutputLearningSource = "begin_task" | "external";
...
export interface ExternalLearningLoopInput {
...
function normalizeLoop(raw: unknown): OutputLearningLoop | null {
  ...
  return {
    ...
    source: source.source === "external" ? "external" : "begin_task",
    runtimeId: boundedLine(source.runtimeId, 120) || "unknown",
  };
}
...
function pullSharedDocumentsAsync(force = false): Promise<{ documents: OutputLearningDocument[]; error?: string }> {
  ...
  const now = Date.now();
  const cacheTtlMs = sharedCacheError ? SHARED_PULL_FAILURE_RETRY_MS : SHARED_PULL_CACHE_MS;
  if (!force && sharedCache && now - sharedCacheAt < cacheTtlMs) {
...
export class OutputLearningStore {
  ...
  ingestExternalLoops(rawLoops: ExternalLearningLoopInput[]): { added: number; total: number; analysis: OutputLearningAnalysis } {
    void this.refreshSharedSnapshot();
    const document = this.read({ pullShared: true });
    ...
    for (const raw of rawLoops) {
      ...
      const conversationKey = raw.conversationKey
        ...
        : `external:${hash([prompt, assistant])}`;
