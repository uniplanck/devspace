...
async function runBuiltinRuntimeCommand(
  ...
  if (
    ...
    return textResponse([
      ...
      "  devspace-runtime jobs start chatgpt-task --prompt <prompt> [--url <chat-url>] [--expect <marker>] [--images <1-4>] [--writing-kernel <auto|on|off>] [--auto-submit] [--timeout-seconds <5-600>] [--keep-tab]", 
...
function runtimeChatGptTaskInput(tokens: string[]): Record<string, unknown> {
  ...
  const writingKernel = runtimeOption(tokens, "--writing-kernel");
